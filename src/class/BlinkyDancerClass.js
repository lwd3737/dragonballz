...
class BlinkyDancerClass extends DancerClass {
  ...
  randomizeColor(){
    const colors = ['red', 'black', 'pink', 'yellow', 'red'];
    let selectedColor = colors[parseInt(Math.random() * 6)];
    let borderStyle = `20px solid ${selectedColor}`
    this.$node.style.borderTop = borderStyle;
  }

  lineup(){
    let innerEl = document.querySelector('#inner');
    this.$node.style.position = 'static';
    innerEl.append(this.$node);
  }
